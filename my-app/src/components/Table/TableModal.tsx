...
export default function TableModal({
  ...
  const editModal = () => {
    const newTableData = tableData.map((data) =>
      formState?.id === data.id ? formState : data
    );
  ...
  return (
    <div className="absolute top-1/3 left-1/4 shadow-lg bg-white w-1/2 h-80">
      <p
        className="text-end px-4 py-2 text-2xl cursor-pointer"
        onClick={hideModal}
      >
        {"×"}
      </p>
